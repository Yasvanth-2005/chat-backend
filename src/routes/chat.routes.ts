...
router.get('/users/:userId/chats', async (req, res) => {
  try {
    ...
    const chats = await Chat.find({ participants: userId })
      .populate('participants', 'displayName status active email phoneNumber photoURL')
      .populate({
        path: 'lastMessage',
      });
...
router.get('/chats/:chatId/messages/:userId', async (req: any, res: any) => {
  try {
    ...
    const deletedEntry = chat?.deletedFor?.find((d: any) => d.userId.equals(userObjectId));
...
router.post('/chats', async (req: any, res: any) => {
  try {
    ...
    if (!userId || !contact?._id || !message) {
      return res.status(400).json({ error: 'User ID, contact ID, and message are required' });
    }
    ...
    const otherParticipants = populatedChat.participants.filter(
      (p: any) => p._id.toString() !== userId.toString() && p.socketId && p.socketId !== ''
    );
...
router.delete('/chats/:chatId/:userId?', async (req: any, res: any) => {
  try {
    ...
    const lastMessageTime = chat.lastMessage ? chat.lastMessage.createdAt : new Date();
...
router.put('/chats/:chatId/messages/:messageId', async (req: any, res: any) => {
  try {
    ...
    if (message.chatId.toString() !== chatId) {
      return res.status(403).json({ error: 'Message does not belong to this chat' });
    }
    ...
    const chat: any = await Chat.findById(chatId).populate('lastMessage');
    const messageN: any = await Message.findById(messageId);
    console.log(chat?.lastMessage, message);
    const lastMessage = chat?.lastMessage?.id === messageN.id ? messageN : null;
...
router.post('/export', async (req: any, res: any) => {
  try {
    ...
    messages.forEach((message: any) => {
      ...
      const fullText = [`${senderName}, ${timestamp}`, `Message: ${messageContent}`].join('\n');
...
router.post('/messages', async (req: any, res: any) => {
  try {
    ...
    const message = await Message.create({
      ...
      replyTo: content.replyTo ? new Types.ObjectId(content.replyTo) : undefined,
    });
...
router.post('/chats/:chatId/exit', async (req: any, res: any) => {
  try {
    ...
    chat.participants = chat.participants.filter((p: any) => p.toString() !== userId.toString());
    ...
    const populatedChat: any = await Chat.findById(chat._id).populate('participants', 'socketId');
